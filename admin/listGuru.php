...
<body id="body-pd">
  ...
  <div>
    <div class="d-flex gap-2">
      <a href="listPerson.php" class="text-dark nav-link">Murid</a>
      <a href="listGuru.php" class="text-dark nav-link active">Guru</a>
    </div>

    <hr>
    <div class="container mt-5">
      <h2>List Nama Guru</h2>
      <table class="table table-bordered table-sm" style="width: 50%;">
        <thead class="table-primary">
          <tr>
            <th scope="col" class="col-1">No</th>
            <th scope="col" class="col-5">Nama Guru</th>
          ...
          foreach ($result as $row) : ?>
            <tr>
              ...
              <td class="d-flex gap-2">
                <!-- Update Form -->
                  <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#updateModal<?= $row['id_user'] ?>" title="Update">
                    <i class="bi bi-pencil"></i>
                  </button>
                
                <!-- Delete Form -->
                  <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteModal<?= $row['id_user'] ?>" title="Delete">
            ...
            <!-- Update Modal -->
            <div class="modal fade" id="updateModal<?= $row['id_user'] ?>" tabindex="-1" aria-labelledby="updateModalLabel<?= $row['id_user'] ?>" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="updateModalLabel<?= $row['id_user'] ?>">Update Siswa</h5>
                  ...
                  <div class="modal-body">
                    <form action="processAdmin.php" method="post">
                      ...
                      <div class="mb-3">
                        <label for="updatedKelas" class="form-label">Updated Nama Guru:</label>
                        <input type="text" class="form-control" name="updateNamaGuru" value="<?= $row['username'] ?>">
                      </div>
            ...
            <div class="modal fade" id="deleteModal<?= $row['id_user'] ?>" tabindex="-1" aria-labelledby="deleteModalLabel<?= $row['id_user'] ?>" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="deleteModalLabel<?= $row['id_user'] ?>">Delete Siswa</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                  </div>
                  <div class="modal-body">
                    ...
                    <form action="processAdmin.php" method="post">
                      <input type="hidden" name="id_user" value="<?= $row['id_user'] ?>">
                      <button type="submit" class="btn btn-danger" name="deleteGuru">Delete</button>
                    </form>
                  </div>
                </div>
              </div>
